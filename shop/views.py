...
def searchMatch(query, item):
    '''return true only if query is match the items '''
...
def search(request):
    ...
    if len(allProds) == 0 or len(query) < 4:
        params = {"msg": "Please make sure to enter a relevent search query"}
...
def checkout(request):
    if (request.method == "POST"):
        ...
        # param_dict["CHECKSUMHASH"] = paytmchecksum.generate_checksum(param_dict, MERCHANT_KEY)
